    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(news !=null){
            ...
            BeanUtils.copyProperties( news , view );//把实体数据重构到view中
                ...
                //级联表
                MeitiEntity meiti = meitiService.selectById(news.getMeitiId());
                if(meiti != null){
    ...
    */
    ...
    public R save(@RequestBody NewsEntity news, HttpServletRequest request){
        ...
        if(false)
            ...
            news.setMeitiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<NewsEntity> queryWrapper = new EntityWrapper<NewsEntity>()
            .eq("meiti_id", news.getMeitiId())
            .eq("news_name", news.getNewsName())
            .eq("news_types", news.getNewsTypes())
            .eq("news_yesno_types", news.getNewsYesnoTypes())
            .eq("news_yesno_text", news.getNewsYesnoText())
            ;
...
//                            newsEntity.setMeitiId(Integer.valueOf(data.get(0)));   //媒体 要改的
...
//                            newsEntity.setNewsYesnoTypes(Integer.valueOf(data.get(0)));   //新闻审核 要改的
//                            newsEntity.setNewsYesnoText(data.get(0));                    //审核结果 要改的
        ...
        NewsEntity news = newsService.selectById(id);
            if(news !=null){
                ...
                //级联表
                    MeitiEntity meiti = meitiService.selectById(news.getMeitiId());
                if(meiti != null){
                    BeanUtils.copyProperties( meiti , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setMeitiId(meiti.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
