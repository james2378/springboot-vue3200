...
public class NewsLiuyanController {
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(newsLiuyan !=null){
            ...
            BeanUtils.copyProperties( newsLiuyan , view );//把实体数据重构到view中
                ...
                if(news != null){
                    BeanUtils.copyProperties( news , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
    ...
    */
    ...
    public R update(@RequestBody NewsLiuyanEntity newsLiuyan, HttpServletRequest request){
...
//            newsLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<NewsLiuyanEntity> queryWrapper = new EntityWrapper<NewsLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        NewsLiuyanEntity newsLiuyanEntity = newsLiuyanService.selectOne(queryWrapper);
        newsLiuyan.setUpdateTime(new Date());
        if(newsLiuyanEntity==null){
...
//                            newsLiuyanEntity.setNewsId(Integer.valueOf(data.get(0)));   //商品 要改的
//                            newsLiuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            newsLiuyanEntity.setNewsLiuyanText(data.get(0));                    //新闻内容 要改的
